...
for i in {0..9}
do
  sed -i -e "54s/.*/    sigma=${sigma[i]}/" alfven_resistive.py
  for j in {0..4}
  do
    sed -i -e "56s/.*/    epsilon=${epsilon[j]}/" alfven_resistive.py
    for kappa in {0..9}
    do
      sed -i -e "57s/.*/    k=${k[kappa]}/" alfven_resistive.py
      for iter in {0..1}
      do
        python alfven_resistive.py && python energy.py
      done
