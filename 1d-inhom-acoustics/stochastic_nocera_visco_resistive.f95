...
DOUBLE PRECISION :: x,u,b,du_dt,db_dt,GRn,Ek
DIMENSION x(N),u(N),b(N),du_dt(N),db_dt(N),GRn(N),Ek(N)
...
do i = 1, N
  ...
  Ek(i) = sigma*sigma*L*dexp(-k*k)
enddo

! call dfftw_plan_dft_c2r_1d_(planb, N, Ek, E, FFTW_ESTIMATE)
...
do i = 1, N
  ...
  do kn = 1, N
    URn_1 = 2.0d0 * pi * rand()
    GRn(i) = GRn(i) + pF*dsqrt(Ek(kn))*dcos(kn*x(i)+URn_1)
  enddo
enddo
...
!IF ( MOD(t,100) == 0 ) THEN
  ...
  ! if (flag == 0 .and. abs(log(bEnergy)) < 5e-1) then
  !   flag = 1
  !   WRITE(10,*) time, log(bEnergy)!, freq(t)
  ! endif
  ! if (flag == 1) then
